...
#define RENDERER_TYPE_ID(x) else if (strcmp(type, #x) == 0) id = BGFX_RENDERER_TYPE_##x
	...
	else return (bgfx_renderer_type_t) luaL_error(L, "Invalid renderer type %s", type);
...
cb_screen_shot(bgfx_callback_interface_t *self, const char* file, uint32_t width, uint32_t height, uint32_t pitch, const void* data, uint32_t size, bool yflip) {
	struct screenshot *s = (screenshot*) malloc(sizeof(*s));
	size_t fn_sz = strlen(file);
	s->name = (char*) malloc(fn_sz + 1);
...
linit(lua_State *L) {
	...
	struct callback *cb = (callback*)lua_newuserdatauv(L, sizeof(*cb), 0);
...
new_vdecl_from_string(lua_State *L, const char *vdecl, size_t sz) {
	struct vertex_layout * layout = (vertex_layout*)lua_newuserdatauv(L, sizeof(*layout), 0);
...
lmemoryBuffer(lua_State *L) {
	...
	if (lua_type(L, 2) == LUA_TTABLE) {
		...
		uint8_t *data = (uint8_t*)newMemory(L, NULL, numvertex*stride);
...
getIndexBuffer(lua_State *L, int idx, int index32) {
	if (lua_type(L, idx) == LUA_TTABLE) {
		...
		if (index32) {
			...
			copy_layout_data(L, "d", 1, 1, n,(uint8_t*) data);
		}
		else {
			void *data = newMemory(L, NULL, n*sizeof(uint16_t));
			copy_layout_data(L, "w", 1, 1, n, (uint8_t*)data);
		}
...
next_vb_handle(lua_State *L, int stream, struct vertexbuffer *vb) {
	...
	case LUA_TUSERDATA:
		vb->handle = 0;
		vb->tb = (transient_buffer*)luaL_checkudata(L, -1, "BGFX_TB");
		break;
	default:
	...
	if (lua_gettop(L) <= 1) {
		...
		if (is_array) {
			...
			if (lua_type(L, lua_base) == LUA_TUSERDATA) {
				vb.handle = 0;
				vb.tb = (transient_buffer*)luaL_checkudata(L, lua_base, "BGFX_TB");
			} else {
...
ENCODER_API(lsetIndexBuffer) {
	...
	if (lua_type(L, 1) == LUA_TUSERDATA){
		tb = (transient_buffer*)luaL_checkudata(L, 1, "BGFX_TB");
	} else {
...
lallocTB(lua_State *L) {
	struct transient_buffer *v = (transient_buffer*)luaL_checkudata(L, 1, "BGFX_TB");
...
lapplyVB(lua_State *L) {
	...
	struct transient_buffer *v = (transient_buffer*)lua_touserdata(L, 1);
...
lnewTransientBuffer(lua_State *L) {
	...
	struct transient_buffer * v = (transient_buffer*)lua_newuserdatauv(L, sizeof(*v) + sz, 0);
	v->cap_v = 0;
	v->cap_i = 0;
...
lallocIDB(lua_State *L) {
	struct lidb *v = (lidb*)luaL_checkudata(L, 1, "BGFX_IDB");
	int num = luaL_checkinteger(L, 2);
...
lpackIDB(lua_State *L) {
	...
	for (i=0;v->format[i];i++) {
		switch(v->format[i]) {
		case 'm':	// matrix 4x4
			ud = (float*)lua_touserdata(L, lidx);
			if (ud == NULL) {
...
lnewInstanceBuffer(lua_State *L) {
	size_t sz;
	const char * format = luaL_checklstring(L, 1, &sz);
	struct lidb * v = (lidb*)lua_newuserdatauv(L, sizeof(*v) + sz, 0);
	v->num = 0;
	int i;
	int stride = 0;
	for (i=0;format[i];i++) {
...
get_ratio(lua_State *L, int idx) {
	bgfx_backbuffer_ratio_t ratio = (bgfx_backbuffer_ratio_t)0;
...
access_string(lua_State *L, const char * access) {
	...
	if (access[0] == 'r') {
		...
		return (bgfx_access_t) luaL_error(L, "Invalid buffer access %s", access);
	}
	if (access[1] != 0) {
...
ENCODER_API(lsetImage) {
	...
	bgfx_texture_format_t format = (bgfx_texture_format_t)0;
...
lgetScreenshot(lua_State *L) {
	...
	struct callback *cb = (callback * )lua_touserdata(L, -1);
