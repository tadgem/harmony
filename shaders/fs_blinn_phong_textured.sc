...
void main()
{
    ...
    {
        output += BlinnPhong_Point(i, v_position, v_normal, ambient, diffuse, spec, shininess);
    }

    for(int i = 0; i < SpotLights_NumActive; i++)
    {
        output += BlinnPhong_Spot(i, v_position, v_normal, ambient, diffuse, spec, shininess);
    }
