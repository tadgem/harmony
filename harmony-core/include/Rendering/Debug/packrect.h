...
template<uint16_t numBlocks> class RectPackCubeT;
template<uint16_t numBlocks> class RectPack2DT
{
public:
	...
	void clear(const Pack2D &_pack)
	{
		...
		uint16_t endx =
				bx::min<uint16_t>(64, (_pack.m_width + m_bw - 1) / m_bw + startx);
		uint16_t endy = bx::min<uint16_t>(
				numBlocks, (_pack.m_height + m_bh - 1) / m_bh + starty);
		uint16_t width = endx - startx;
		const uint64_t mask =
				(width == 64 ? UINT64_MAX : (UINT64_C(1) << width) - 1) << startx;
