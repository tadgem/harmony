...
namespace harmony {
    ...
    public:
        ...
        void drawLineList(Channel channel, uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices = 0,
                          const uint16_t *_indices = NULL);

        void drawTriList(Channel channel, uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices = 0,
                         const uint16_t *_indices = NULL);
        ...
        void drawCircle(Channel channel, const bx::Vec3 &_normal, const bx::Vec3 &_center, float _radius,
                        float _weight = 0.0f);

        void drawCircle(Channel channel, Axis::Enum _axis, float _x, float _y, float _z, float _radius,
                        float _weight = 0.0f);

        void drawQuad(Channel channel, const bx::Vec3 &_normal, const bx::Vec3 &_center, float _size);

        void
        drawQuad(Channel channel, SpriteHandle _handle, const bx::Vec3 &_normal, const bx::Vec3 &_center, float _size);

        void drawQuad(Channel channel, bgfx::TextureHandle _handle, const bx::Vec3 &_normal, const bx::Vec3 &_center,
                      float _size);
        ...
        void
        drawAxis(Channel channel, float _x, float _y, float _z, float _len = 1.0f, Axis::Enum _highlight = Axis::Count,
                 float _thickness = 0.0f);

        void drawGrid(Channel channel, const bx::Vec3 &_normal, const bx::Vec3 &_center, uint32_t _size = 20,
                      float _step = 1.0f);

        void
        drawGrid(Channel channel, Axis::Enum _axis, const bx::Vec3 &_center, uint32_t _size = 20, float _step = 1.0f);
