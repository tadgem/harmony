...
namespace
{
	...
	static int nvgRenderCreate(void *_userPtr)
	{
		...
		gl->prog = bgfx::createProgram(
				...
				bgfx::createEmbeddedShader(s_embeddedShaders, type, "fs_nanovg_fill"), true
		);
	...
	)
	{
		...
		tex->id = bgfx::createTexture2D(
				tex->width, tex->height, false, 1,
				NVG_TEXTURE_RGBA == _type ? bgfx::TextureFormat::RGBA8 : bgfx::TextureFormat::R8, BGFX_SAMPLER_NONE
		);
	...
	static int
	nvgRenderUpdateTexture(void *_userPtr, int image, int x, int y, int w, int h, const unsigned char *data)
	{
		struct GLNVGcontext *gl = (struct GLNVGcontext *) _userPtr;
	...
	static int glnvg__convertPaint(
			struct GLNVGcontext *gl, struct GLNVGfragUniforms *frag, struct NVGpaint *paint, struct NVGscissor *scissor,
			float width, float fringe
	)
	{
		struct GLNVGtexture *tex = NULL;
		float invxform[6] = {};
	...
	static GLNVGblend glnvg__blendCompositeOperation(NVGcompositeOperationState op)
	{
		...
		if (blend.srcRGB == BGFX_STATE_NONE || blend.dstRGB == BGFX_STATE_NONE || blend.srcAlpha == BGFX_STATE_NONE ||
			blend.dstAlpha == BGFX_STATE_NONE)
		{
	...
	static void nvgRenderFlush(void *_userPtr)
	{
		...
		if (gl->ncalls > 0)
		{
			...
			for (uint32_t ii = 0, num = gl->ncalls; ii < num; ++ii)
			{
				...
				gl->state =
						BGFX_STATE_BLEND_FUNC_SEPARATE(blend->srcRGB, blend->dstRGB, blend->srcAlpha, blend->dstAlpha)
	...
	static void nvgRenderFill(
			void *_userPtr, NVGpaint *paint, NVGcompositeOperationState compositeOperation, NVGscissor *scissor,
	...
	static void nvgRenderStroke(
			void *_userPtr, struct NVGpaint *paint, NVGcompositeOperationState compositeOperation,
			struct NVGscissor *scissor, float fringe, float strokeWidth, const struct NVGpath *paths, int npaths
	)
	{
		struct GLNVGcontext *gl = (struct GLNVGcontext *) _userPtr;
		struct GLNVGcall *call = glnvg__allocCall(gl);
	...
	static void
	nvgRenderTriangles(void *_userPtr, struct NVGpaint *paint, NVGcompositeOperationState compositeOperation,
...
NVGLUframebuffer *
nvgluCreateFramebuffer(NVGcontext *ctx, int32_t width, int32_t height, int32_t imageFlags, bgfx::ViewId viewId)
{
	...
	bgfx::TextureHandle textures[] =
			{
					...
					bgfx::createTexture2D(_width, _height, false, 1, bgfx::TextureFormat::D24S8,
										  BGFX_TEXTURE_RT | BGFX_TEXTURE_RT_WRITE_ONLY)
			};
	bgfx::FrameBufferHandle fbh = bgfx::createFrameBuffer(
			BX_COUNTOF(textures), textures, true
	);
...
NVGLUframebuffer *nvgluCreateFramebuffer(NVGcontext *_ctx, int32_t _imageFlags)
{
	...
	bgfx::TextureHandle textures[] =
			{
					bgfx::createTexture2D(bgfx::BackbufferRatio::Equal, false, 1, bgfx::TextureFormat::RGBA8,
										  BGFX_TEXTURE_RT),
					bgfx::createTexture2D(bgfx::BackbufferRatio::Equal, false, 1, bgfx::TextureFormat::D24S8,
										  BGFX_TEXTURE_RT | BGFX_TEXTURE_RT_WRITE_ONLY)
			};
	bgfx::FrameBufferHandle fbh = bgfx::createFrameBuffer(
			BX_COUNTOF(textures), textures, true
