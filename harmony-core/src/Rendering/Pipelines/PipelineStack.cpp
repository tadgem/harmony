...
void harmony::PipelineStack::PreUpdate(entt::registry &registry, WeakRef<View> view) {
    ...
    for (int p = 0; p < m_PipelineStack.size(); p++) {
        ...
        pipeline->PreUpdate(registry, view, p_StackViewIDs[pipeline->m_Handle.Name],
                            p_StackData[pipeline->m_Handle.Name]);
...
harmony::PipelineStack::PostUpdate(entt::registry &registry, WeakRef<View> view) {
    ...
    for (int p = 0; p < m_PipelineStack.size(); p++) {
        ...
        pipeline->PostUpdate(registry, view, p_StackViewIDs[pipeline->m_Handle.Name],
                             p_StackData[pipeline->m_Handle.Name]);
    }
...
void harmony::PipelineStack::MovePostProcessStage(const std::string &postProcessStageName, bool moveUp) {
    ...
    if (shiftDir + indexToShift < m_PostProcessPipelineStack.size()) {
        std::iter_swap(m_PostProcessPipelineStack.begin() + indexToShift,
                       m_PostProcessPipelineStack.begin() + (indexToShift + shiftDir));
    }
}
